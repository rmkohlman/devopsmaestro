...
func ParseYAMLMultiple(data []byte) ([]*Package, error) {
	...
	for {
		...
		if py.Metadata.Name == "" && len(py.Spec.Plugins) == 0 && len(py.Spec.Prompts) == 0 && len(py.Spec.Profiles) == 0 {
			continue
		}
...
func validatePackageYAML(py *PackageYAML) error {
	...
	// Validate kind
	if py.Kind != "" && py.Kind != "TerminalPackage" {
		return fmt.Errorf("invalid kind: expected 'TerminalPackage', got %q", py.Kind)
	}
	...
	// Validate prompt names (basic check)
	for i, promptName := range py.Spec.Prompts {
		if promptName == "" {
			return fmt.Errorf("prompt name at index %d cannot be empty", i)
		}
	}

	// Validate profile names (basic check)
	for i, profileName := range py.Spec.Profiles {
		if profileName == "" {
			return fmt.Errorf("profile name at index %d cannot be empty", i)
		}
	}

	// Validate extends field (if present, must not be empty and not self-reference)
