...
type PromptRenderer interface {
	...
	// RenderComposed generates config file content from a ComposedPrompt
	// using the provided palette for color resolution.
	RenderComposed(composed *composer.ComposedPrompt, name string, pal *palette.Palette) (string, error)

	// RenderComposedToFile renders a ComposedPrompt and writes to a file path.
	RenderComposedToFile(composed *composer.ComposedPrompt, name string, pal *palette.Palette, path string) error
}
...
// RenderComposed generates starship.toml content from a ComposedPrompt.
func (r *StarshipRenderer) RenderComposed(composed *composer.ComposedPrompt, name string, pal *palette.Palette) (string, error) {
	if composed == nil {
		return "", fmt.Errorf("composed prompt is nil")
	}
	if pal == nil {
		return "", fmt.Errorf("palette is nil")
	}
	...
	toml.WriteString(fmt.Sprintf("# Prompt: %s (composed)\n", name))
	...
	// Write module sections
	r.writeComposedModuleSections(&toml, composed, pal)

	return toml.String(), nil
}

// RenderComposedToFile renders a ComposedPrompt and writes to a file path.
func (r *StarshipRenderer) RenderComposedToFile(composed *composer.ComposedPrompt, name string, pal *palette.Palette, path string) error {
	content, err := r.RenderComposed(composed, name, pal)
	if err != nil {
		return fmt.Errorf("failed to render: %w", err)
	}
...
// writeComposedModuleSections writes all module configuration sections from a ComposedPrompt.
func (r *StarshipRenderer) writeComposedModuleSections(toml *strings.Builder, composed *composer.ComposedPrompt, pal *palette.Palette) {
	// Sort module names for consistent output
	moduleNames := make([]string, 0, len(composed.Modules))
	for name := range composed.Modules {
		moduleNames = append(moduleNames, name)
	}
	sort.Strings(moduleNames)

	for _, name := range moduleNames {
		module := composed.Modules[name]
		r.writeComposedModuleSection(toml, name, &module, pal)
	}
}

// writeComposedModuleSection writes a single module configuration section from a ComposedPrompt module.
func (r *StarshipRenderer) writeComposedModuleSection(toml *strings.Builder, name string, module *composer.ModuleConfig, pal *palette.Palette) {
	toml.WriteString(fmt.Sprintf("[%s]\n", name))

	// Write disabled status - important for modules like 'os' that are disabled by default
	if module.Disabled {
	...
	if module.Style != "" {
		...
		toml.WriteString(fmt.Sprintf("style = \"%s\"\n", escapeTOMLString(resolvedStyle)))
	}

	// Write format (with theme variables resolved)
	if module.Format != "" {
		resolvedFormat := r.resolveThemeVars(module.Format, pal)
		toml.WriteString(fmt.Sprintf("format = \"%s\"\n", escapeTOMLString(resolvedFormat)))
	}

	// Write symbol
	if module.Symbol != "" {
		toml.WriteString(fmt.Sprintf("symbol = \"%s\"\n", escapeTOMLString(module.Symbol)))
	}

	// Write additional options (excluding keys already written above to avoid duplicates)
	for key, value := range module.Options {
		switch key {
		case "disabled", "style", "format", "symbol":
			continue // Skip keys handled explicitly above
		}
...
func (r *StarshipRenderer) writePaletteSection(toml *strings.Builder, pal *palette.Palette) {
	...
	// Write Catppuccin-style color aliases for styles using names like "bg:peach", "fg:crust"
	// These map Catppuccin names to the palette's actual color values
	// If promptColors are defined, they override the default mappings
	catppuccinColors := map[string]string{
		"crust":     pal.Get(palette.ColorBg),
		"mantle":    pal.Get(palette.ColorBgDark),
		"base":      pal.Get(palette.ColorBg),
		"surface0":  pal.Get(palette.ColorBgHighlight),
		"surface1":  pal.Get(palette.ColorBgHighlight),
		"surface2":  pal.Get(palette.ColorBgHighlight),
		"text":      pal.Get(palette.ColorFg),
		"subtext0":  pal.Get(palette.ColorComment),
		"subtext1":  pal.Get(palette.ColorComment),
		"overlay0":  pal.Get(palette.ColorBorder),
		"overlay1":  pal.Get(palette.ColorBorder),
		"overlay2":  pal.Get(palette.ColorBorder),
		"lavender":  pal.Get(palette.TermBrightMagenta),
		"sky":       pal.Get(palette.TermCyan),
		"sapphire":  pal.Get(palette.TermBlue),
		"teal":      pal.Get(palette.ColorTeal),
		"peach":     pal.Get(palette.ColorOrange),
		"maroon":    pal.Get(palette.TermRed),
		"pink":      pal.Get(palette.ColorPink),
		"mauve":     pal.Get(palette.TermMagenta),
		"flamingo":  pal.Get(palette.ColorPink),
		"rosewater": pal.Get(palette.ColorFg),
		"red":       pal.Get(palette.TermRed),
		"green":     pal.Get(palette.TermGreen),
		"yellow":    pal.Get(palette.TermYellow),
		"blue":      pal.Get(palette.TermBlue),
	}

	// Apply prompt color overrides if available
	// This allows themes to define custom colors for prompt segments
	// independent of the Neovim syntax highlighting colors
	if pal.HasPromptColors() {
		for key, color := range pal.PromptColors {
			if color != "" {
				catppuccinColors[key] = color
			}
		}
	}

	// Sort and write Catppuccin colors
	catKeys := make([]string, 0, len(catppuccinColors))
	for k := range catppuccinColors {
		catKeys = append(catKeys, k)
	}
	sort.Strings(catKeys)

	for _, key := range catKeys {
		if color := catppuccinColors[key]; color != "" {
			toml.WriteString(fmt.Sprintf("%s = \"%s\"\n", key, color))
		}
	}
...
func (r *StarshipRenderer) writeValue(toml *strings.Builder, key string, value any, moduleName string, pal *palette.Palette) {
	...
	case []any:
		// Check if this is an array of tables (array of maps)
		if len(v) > 0 {
			if _, isMap := v[0].(map[string]any); isMap {
				// Write as TOML array of tables: [[module.key]]
				for _, item := range v {
					if m, ok := item.(map[string]any); ok {
						...
						for _, subKey := range subKeys {
							r.writeValue(toml, subKey, m[subKey], moduleName+"."+key, pal)
						}
					}
				}
				return
