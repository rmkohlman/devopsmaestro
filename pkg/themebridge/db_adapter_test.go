...
func TestDBStoreAdapter_ImplementsStore(t *testing.T) {
	var _ theme.Store = (*DBStoreAdapter)(nil)
}
...
func TestDBStoreAdapter_Save(t *testing.T) {
	...
	th.Description = "Updated description"
...
func TestDBStoreAdapter_Get(t *testing.T) {
	...
	// Test successful get
	th, err := adapter.Get("catppuccin")
	if err != nil {
		t.Fatalf("Get failed: %v", err)
	}
	if th.Name != "catppuccin" {
		t.Errorf("Expected name 'catppuccin', got '%s'", th.Name)
	}
	if th.Description != "Soothing pastel theme" {
		t.Errorf("Expected description 'Soothing pastel theme', got '%s'", th.Description)
	}
...
func TestDBModelToTheme_BasicFields(t *testing.T) {
	...
	th := dbModelToTheme(db)

	if th.Name != "catppuccin" {
		t.Errorf("Name mismatch: %s", th.Name)
	}
	if th.Description != "Soothing pastel theme" {
		t.Errorf("Description mismatch: %s", th.Description)
	}
	if th.Author != "catppuccin" {
		t.Errorf("Author mismatch: %s", th.Author)
	}
	if th.Category != "dark" {
		t.Errorf("Category mismatch: %s", th.Category)
	}
	if th.Plugin.Repo != "catppuccin/nvim" {
		t.Errorf("Plugin.Repo mismatch: %s", th.Plugin.Repo)
	}
	if th.Plugin.Branch != "main" {
		t.Errorf("Plugin.Branch mismatch: %s", th.Plugin.Branch)
	}
	if th.Plugin.Tag != "v1.0.0" {
		t.Errorf("Plugin.Tag mismatch: %s", th.Plugin.Tag)
	}
	if th.Style != "mocha" {
		t.Errorf("Style mismatch: %s", th.Style)
	}
	if !th.Transparent {
		t.Error("Transparent should be true")
	}
...
func TestDBModelToTheme_JSONFields(t *testing.T) {
	...
	if len(th.Colors) != 2 {
		t.Errorf("Expected 2 colors, got %d", len(th.Colors))
	}
	if th.Colors["bg"] != "#1e1e2e" {
		t.Errorf("Colors[bg] mismatch: %s", th.Colors["bg"])
	}
	if len(th.Options) != 1 {
		t.Errorf("Expected 1 option, got %d", len(th.Options))
	}
}
...
func TestStore_ThroughInterface(t *testing.T) {
	...
	var s theme.Store = NewDBStoreAdapter(mock)
	...
	retrieved, err := s.Get("catppuccin")
	if err != nil {
...
func TestStore_Swappability(t *testing.T) {
	...
	testStore := func(s theme.Store, name string) {
		th := &theme.Theme{
			Name: "test-theme",
			Plugin: theme.ThemePlugin{
				Repo: "test/repo",
		...
		// Get
		retrieved, err := s.Get("test-theme")
		if err != nil {
		...
		// SetActive
		if err := s.SetActive("test-theme"); err != nil {
		...
		// Delete
		if err := s.Delete("test-theme"); err != nil {
...
func TestIntegration_ActiveThemeOperations(t *testing.T) {
	...
	// Create two themes
	themes := []*theme.Theme{
		{Name: "catppuccin", Category: "dark", Plugin: theme.ThemePlugin{Repo: "catppuccin/nvim"}},
		{Name: "tokyonight", Category: "dark", Plugin: theme.ThemePlugin{Repo: "folke/tokyonight.nvim"}},
	}

	for _, th := range themes {
		if err := adapter.Save(th); err != nil {
			t.Fatalf("Save %s failed: %v", th.Name, err)
		}
	}
...
func TestIntegration_ListOperations(t *testing.T) {
	...
	themes := []*theme.Theme{
		{Name: "catppuccin", Category: "dark", Plugin: theme.ThemePlugin{Repo: "catppuccin/nvim"}},
		{Name: "tokyonight", Category: "dark", Plugin: theme.ThemePlugin{Repo: "folke/tokyonight.nvim"}},
		{Name: "solarized", Category: "light", Plugin: theme.ThemePlugin{Repo: "maxmx03/solarized.nvim"}},
		{Name: "gruvbox", Category: "dark", Plugin: theme.ThemePlugin{Repo: "ellisonleao/gruvbox.nvim"}},
	}
