// Package themebridge provides a database adapter that bridges the theme.Store
...
func (a *DBStoreAdapter) Get(name string) (*theme.Theme, error) {
...
func (a *DBStoreAdapter) List() ([]*theme.Theme, error) {
	dbThemes, err := a.store.ListThemes()
	if err != nil {
		return nil, fmt.Errorf("failed to list themes: %w", err)
	}

	themes := make([]*theme.Theme, len(dbThemes))
...
// Save stores a theme.
func (a *DBStoreAdapter) Save(th *theme.Theme) error {
	if err := th.Validate(); err != nil {
		return fmt.Errorf("invalid theme: %w", err)
	}

	// Check if theme already exists
	existing, err := a.store.GetThemeByName(th.Name)
	if err == nil {
		// Theme exists, update it
		dbTheme := themeToDBModel(th)
...
func (a *DBStoreAdapter) GetActive() (*theme.Theme, error) {
...
func themeToDBModel(t *theme.Theme) *models.NvimThemeDB {
...
func dbModelToTheme(db *models.NvimThemeDB) *theme.Theme {
	t := &theme.Theme{
		Name: db.Name,
...
// Ensure DBStoreAdapter implements theme.Store interface
