...
func (ds *SQLDataStore) CreateWorkspace(workspace *models.Workspace) error {
	...
	query := fmt.Sprintf(`INSERT INTO workspaces (app_id, name, slug, description, image_name, status, ssh_agent_forwarding, nvim_structure, nvim_plugins, theme, terminal_prompt, terminal_plugins, terminal_package, git_repo_id, env, build_config, created_at, updated_at) 
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, %s, %s)`, ds.queryBuilder.Now(), ds.queryBuilder.Now())

	result, err := ds.driver.Execute(query, workspace.AppID, workspace.Name, workspace.Slug, workspace.Description, workspace.ImageName, workspace.Status, workspace.SSHAgentForwarding, workspace.NvimStructure, workspace.NvimPlugins, workspace.Theme, workspace.TerminalPrompt, workspace.TerminalPlugins, workspace.TerminalPackage, workspace.GitRepoID, workspace.Env, workspace.BuildConfig)
	if err != nil {
...
func (ds *SQLDataStore) GetWorkspaceByName(appID int, name string) (*models.Workspace, error) {
	...
	query := `SELECT id, app_id, name, slug, description, image_name, container_id, status, ssh_agent_forwarding, nvim_structure, nvim_plugins, theme, terminal_prompt, terminal_plugins, terminal_package, git_repo_id, env, build_config, created_at, updated_at 
...
func (ds *SQLDataStore) UpdateWorkspace(workspace *models.Workspace) error {
	query := fmt.Sprintf(`UPDATE workspaces SET name = ?, slug = ?, description = ?, image_name = ?, container_id = ?, 
		status = ?, ssh_agent_forwarding = ?, nvim_structure = ?, nvim_plugins = ?, theme = ?, terminal_prompt = ?, terminal_plugins = ?, terminal_package = ?, git_repo_id = ?, env = ?, build_config = ?, updated_at = %s WHERE id = ?`,
		ds.queryBuilder.Now())

	_, err := ds.driver.Execute(query, workspace.Name, workspace.Slug, workspace.Description, workspace.ImageName,
		workspace.ContainerID, workspace.Status, workspace.SSHAgentForwarding, workspace.NvimStructure, workspace.NvimPlugins, workspace.Theme, workspace.TerminalPrompt, workspace.TerminalPlugins, workspace.TerminalPackage, workspace.GitRepoID, workspace.Env, workspace.BuildConfig, workspace.ID)
	if err != nil {
...
func (ds *SQLDataStore) FindWorkspaces(filter models.WorkspaceFilter) ([]*models.WorkspaceWithHierarchy, error) {
	...
	query := `SELECT 
		w.id, w.app_id, w.name, w.description, w.image_name, w.container_id, w.status, w.nvim_structure, w.nvim_plugins, w.theme, w.terminal_prompt, w.terminal_plugins, w.terminal_package, w.slug, w.ssh_agent_forwarding, w.git_repo_id, w.env, w.build_config, w.created_at, w.updated_at,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&workspace.NvimPlugins, &workspace.Theme, &workspace.TerminalPrompt, &workspace.TerminalPlugins, &workspace.TerminalPackage, &workspace.Slug, &workspace.SSHAgentForwarding, &workspace.GitRepoID, &workspace.Env, &workspace.BuildConfig, &workspace.CreatedAt, &workspace.UpdatedAt,
			// App fields
