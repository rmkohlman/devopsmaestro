...
func (ds *SQLDataStore) CreateEcosystem(ecosystem *models.Ecosystem) error {
	query := fmt.Sprintf(`INSERT INTO ecosystems (name, description, theme, build_args, created_at, updated_at) 
		VALUES (?, ?, ?, ?, %s, %s)`, ds.queryBuilder.Now(), ds.queryBuilder.Now())

	result, err := ds.driver.Execute(query, ecosystem.Name, ecosystem.Description, ecosystem.Theme, ecosystem.BuildArgs)
	if err != nil {
...
func (ds *SQLDataStore) GetEcosystemByName(name string) (*models.Ecosystem, error) {
	...
	query := `SELECT id, name, description, theme, build_args, created_at, updated_at FROM ecosystems WHERE name = ?`

	row := ds.driver.QueryRow(query, name)
	if err := row.Scan(&ecosystem.ID, &ecosystem.Name, &ecosystem.Description, &ecosystem.Theme, &ecosystem.BuildArgs, &ecosystem.CreatedAt, &ecosystem.UpdatedAt); err != nil {
...
func (ds *SQLDataStore) GetEcosystemByID(id int) (*models.Ecosystem, error) {
	ecosystem := &models.Ecosystem{}
	query := `SELECT id, name, description, theme, build_args, created_at, updated_at FROM ecosystems WHERE id = ?`
...
func (ds *SQLDataStore) UpdateEcosystem(ecosystem *models.Ecosystem) error {
	query := fmt.Sprintf(`UPDATE ecosystems SET name = ?, description = ?, theme = ?, build_args = ?, updated_at = %s WHERE id = ?`,
		ds.queryBuilder.Now())

	_, err := ds.driver.Execute(query, ecosystem.Name, ecosystem.Description, ecosystem.Theme, ecosystem.BuildArgs, ecosystem.ID)
	if err != nil {
...
func (ds *SQLDataStore) ListEcosystems() ([]*models.Ecosystem, error) {
	query := `SELECT id, name, description, theme, build_args, created_at, updated_at FROM ecosystems ORDER BY name`
	...
	for rows.Next() {
		ecosystem := &models.Ecosystem{}
		if err := rows.Scan(&ecosystem.ID, &ecosystem.Name, &ecosystem.Description, &ecosystem.Theme, &ecosystem.BuildArgs, &ecosystem.CreatedAt, &ecosystem.UpdatedAt); err != nil {
			return nil, fmt.Errorf("failed to scan ecosystem: %w", err)
		}
