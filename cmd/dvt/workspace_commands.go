...
var shellGenerateWorkspaceCmd = &cobra.Command{
	...
	Short: "Generate a .zshrc.workspace file",
	Long: `Generate a workspace-scoped .zshrc file that composites:
  - Host ~/.zshrc as base (if present)
...
Examples:
  ...
  dvt shell generate-workspace --no-host-zshrc
...
func runShellGenerateWorkspace(cmd *cobra.Command, args []string) error {
	...
	// Host zshrc
	noHost, _ := cmd.Flags().GetBool("no-host-zshrc")
	if noHost {
		f := false
		config.IncludeHostZshrc = &f
	}

	hostPath, _ := cmd.Flags().GetString("host-zshrc")
	if hostPath != "" {
		config.HostZshrcPath = hostPath
	}
	...
	if err != nil {
		return fmt.Errorf("failed to generate .zshrc.workspace: %w", err)
	}
	...
	if outputPath != "" {
		...
		render.Successf("Generated .zshrc.workspace at %s", outputPath)
		return nil
	}
...
func init() {
	...
	shellGenerateWorkspaceCmd.Flags().Bool("no-host-zshrc", false, "Skip including host ~/.zshrc")
	shellGenerateWorkspaceCmd.Flags().String("host-zshrc", "", "Path to host .zshrc (default: ~/.zshrc)")
