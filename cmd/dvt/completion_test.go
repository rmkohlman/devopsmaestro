...
func TestCompletionCommand_Exists(t *testing.T) {
	...
	assert.True(t, found, "dvt should have a 'completion' command registered")
}
...
func TestCompletionCommand_ProducesOutput(t *testing.T) {
	...
	for _, shell := range shells {
		t.Run(shell, func(t *testing.T) {
			...
			err := rootCmd.Execute()
			require.NoError(t, err, "dvt completion %s should not error", shell)
			assert.NotEmpty(t, buf.String(), "dvt completion %s should produce non-empty output", shell)
		})
	}
}
...
func TestCompletionCommand_HelpText(t *testing.T) {
	assert.Contains(t, completionCmd.Long, "source <(dvt completion",
