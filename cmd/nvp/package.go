...
// packageGetCmd shows package details or lists all packages
var packageGetCmd = &cobra.Command{
	Use:     "get [name]",
	Aliases: []string{"list"},
	Short:   "Get package(s)",
	Long: `Get packages from the library.

With no arguments, lists all available packages.
With a name argument, shows details of a specific package including all plugins from inheritance.
...
func outputPackages(packages []*nvimpackage.Package, format string, wide bool, lib *packagelibrary.Library) error {
	...
	case "table", "":
		...
		if wide {
			tb = render.NewTableBuilder("NAME", "TYPE", "CATEGORY", "PLUGINS", "EXTENDS", "DESCRIPTION")
		} else {
		...
		for _, p := range packages {
			...
			if wide {
				tb.AddRow(p.Name, "library", p.Category, fmt.Sprintf("%d", pluginCount), p.Extends, desc)
			} else {
